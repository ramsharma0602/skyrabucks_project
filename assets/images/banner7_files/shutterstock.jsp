	...
	usi_analytics = {
		...
		usi_app.main = function () {
			try { // General
				...
				usi_app.flex_destination_url = usi_app.get_flex_destination_url();
		...
		usi_app.flex_lang_check = function() {
			return !(usi_app.is_hindi_language || usi_app.is_tamil_language || usi_app.is_targus_site || usi_app.is_marathi_language);
		};
		...
		usi_app.load = function () {
			...
			try {
				...
				if (!usi_app.suppress_page) {
					...
					if (!usi_cookies.value_exists("usi_active_cart") && usi_app.is_creative_flow_page) {
						if (usi_app.force_group == '') {
							usi_app.force_group = Math.random() < 0.5 ? 0 : 1;
						}
						usi_split_test.instantiate('46273', usi_app.force_group);
						if (usi_split_test.get_group('46273') == '1') {
							var split_key = usi_app.get_split_key("cf", "flex", 0.5, false);
							if (split_key == 'cf') {
								//CF
								usi_commons.log('[ load ] * * * Load Creative Flow Page CF Logo [language key: ' + language_key + '] * * *');
								usi_commons.load_view("n9DDBO5WBTEHS2xF4DsyksV", "45531", usi_commons.device + language_key);
							} else {
								//Flex
								usi_commons.log('[ load ] * * * Load Creative Flow Page Flex Logo [language key: ' + language_key + '] * * *');
								usi_commons.load_view("R1TVctoTYRZ2okxHT6TDLb0", "45533", usi_commons.device + language_key);
							}
						} else {
							//Control
						}
						...
						if (usi_app.is_image_page || usi_app.is_image_in_cart) {
							if (usi_app.check_sub_total_price(49) && usi_app.is_flexable) {
								//Flex
								...
								if(usi_date.is_between('2023-05-15T00:00', '2023-05-30T23:59:59')) return; //Scheduled pause (client request)
							...
							if (usi_app.check_sub_total_price(79) && usi_app.is_flexable) {
								usi_app.messaging = usi_app.get_messaging("video_flex");
								if (usi_app.messaging && usi_app.messaging.header1) {
								...
								if (usi_app.is_cart_page) {
									...
									//TODO make for english only when going live and 50/50 test
							...
							if (usi_app.check_sub_total_price(49) && usi_app.is_flexable) {
								...
								if(usi_date.is_between('2023-05-15T00:00', '2023-05-30T23:59:59')) return; //Scheduled pause (client request)
								var split_key = usi_app.get_split_key("video", "static", 0.5, false);
								if (split_key == 'video') {
									//CF
									...
									usi_app.messaging = usi_app.get_messaging("music_tenpercent");
									if (usi_app.messaging && usi_app.messaging.header1) {
										usi_commons.log('[ load ] * * * Load 10% off Music Static BG [language key: ' + language_key + '] * * *');
		...
		usi_app.get_flex_destination_url = function(){
			var destination_url = "https://www.shutterstock.com/explore/flex-subscriptions";
