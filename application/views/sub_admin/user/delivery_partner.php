    ...
    <div class="container-fluid">
      ...
      <div class="row">
      	<div class="col-lg-12" style="padding: 10px;">
           <button type="button" id="addSupportTeam" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#exampleModal1"><i class="fa fa-plus-circle"></i> Create Delivery Partner</button>
      	</div>
       <div class="col-lg-12">
		  ...
		  <thead id="myHead" >
		    <tr>
		      ...
		      <th scope="col">Delivery Partner  Email</th>
		      <th scope="col">Delivery Partner  Mobile No.</th>
          <th scope="col">Delivery Partner  City Name</th>
        ...
        <form id="myform" action="">
         <input type="hidden" name="user_type_id" value= "6">
          <div class="form-group">
            <label for="exampleFormControlSelect2">Delivery Partner Name</label>
           <input type="text" name="name" class="form-control" placeholder="Delivery Partner Name">
          </div>
          
           <div class="form-group">
            <label for="exampleFormControlSelect2">Delivery Partner Email</label>
           <input type="text" name="email" class="form-control" placeholder="Delivery Partner Email">
          </div>


              <div class="form-group">
                <label for="exampleFormControlSelect2">Delivery Partner Mobile No.</label>
               <input type="int" name="mobile" class="form-control" placeholder="Delivery Partner Mobile No.">
              </div>
...
$(function showallData(){ 
  showallData();
          ...
          $('#myform').attr('action','<?php echo base_url(); ?>api/Delivery_partner/Add_user');
      });
...
$('#brand_data').on('change','#is_active', function() {
   ...
   var $this = $( this ),
        user_id = $this.val();
        $.ajax({
          url:"<?php echo base_url('api/Delivery_partner/fetch_delivery_partner/');?>"+user_id,
          method:"POST",
          async: false,
          dataType: 'json',  
            success:function(response){
 ...
 function showallData(){
          ...
          $.ajax({
                type: 'ajax',
                method:'get',
                url: "<?php echo base_url() ?>api/Delivery_partner/fetch_delivery_partner",
                async: false,
                data : {
                  user_type_id:6
                },
                         ...
                         var i=x+1
                                    ...
                                    if (data[x].profile_images!=null) {
                                      var image_path='<a href="javascript:;" style="margin: 10px;" class="item-image" data="'+data[x].Support_team_id+'"><img src="<?php echo base_url(); ?>uploads/users_profile/'+data[x].profile_images+'" style="height:50px; width:50px;" ></a>';
                                        ...
                                        var image_path='<a href="javascript:;" style="margin: 10px;" class="item-image" data="'+data[x].userm_id+'"><img src="<?php echo base_url(); ?>uploads/users_profile/default.png" style="height:50px; width:50px;" ></a>';
                                        }
                              ...
                              html+='<tr>'+
                                  '<td>'+i+'</td>'+
                                   ...
                                   '<td>'+data[x].name+'</td>'+  
                                    '<td>'+data[x].email+'</td>'+
                                    '<td>'+data[x].mobile+'</td>'+
                                    '<td>'+'<a  style="margin: 5px;" href="<?php echo base_url('Super_admin/Cities_and_delivery_partner/'); ?>'+data[x].delivery_partner_id+'" class="btn btn-sm btn-outline-secondary"><i class="fas fa-plus-circle"></i> Cities</a>'+'</td>'+
