...
<script type="text/javascript">
 ...
 $('#btnadd').click(function(){
   ...
   $('#myform').attr('action','<?php echo base_url(); ?>api/Cities_and_support_team/add_city');
 });
...
$('#btnSave').click(function(){
   ...
   // console.log(data); 
    $.ajax({
        ...
        url: url,
        data: {
          support_team_id:<?php echo $support_team_id ?>,
          city_id:city_id
        },
        async: false,
        dataType: 'json',
...
$('#sub_category_data').on('click','.item-delete',function(){
    ...
    $('#btdelete').unbind().click(function(){
        $.ajax({
            ...
            method: 'delete',
            async: false,
            url: '<?php echo base_url(); ?>api/Cities_and_support_team/delete_location/'+id,
            data: {id: id},
            dataType: 'json',
            success: function(response)
            {
...
function all_category(){
    $.ajax({
          url:"<?php echo base_url().('api/Admin_city/fetch_all_cities');?>",
          method:"get",
          async: false,
          dataType: 'json',  
            success:function(response){
 ...
 function showallData(){
            ...
            $.ajax({
                type: 'ajax',
                url: "<?php echo base_url() ?>api/Cities_and_support_team/fetch_all_location/",
                async: false,
                dataType: 'json',
                success: function(response) {
                    // body...
                    ...
                    for (x =0 ; x<data.length; x++) {
                      var i=x+1
                       html+='<tr>'+
                               ...
                               '<a href="javascript:;" style="margin: 10px;" class="btn btn-sm btn-outline-danger item-delete" data="'+data[x].cs_id+'"><i class="fas fa-trash-alt"></i></a>'+
                               '</td>'+
                           '</tr>';
