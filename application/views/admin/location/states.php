...
$(function showallData(){ 
  showallData();
          ...
          $('#myform')[0].reset();
          $('#btnSave').html('Submit');
          $('#exampleModal1').find('.modal-title').text('Add States');
          $('#myform').attr('action','<?php echo base_url(); ?>api/Location/Add_states');
      });
...
$('#brand_data').on('change','#is_active', function() {
   ...
   var $this = $( this ),
        id = $this.val();
        $.ajax({
          url:"<?php echo base_url('api/Location/states_is_active/');?>"+id,
          method:"POST",
          async: false,
          dataType: 'json',  
            success:function(response){
...
$('#brand_data').on('click','.item-delete',function(){
    ...
    $('#myModal_for_delete_message').find('.modal-title').text('Delete States');
    ...
    $('#btdelete').unbind().click(function(){
        $.ajax({
            ...
            method: 'delete',
            async: false,
            url: '<?php echo base_url(); ?>api/Location/delete_states/'+id,
            data: {id: id},
            dataType: 'json',
            success: function(response)
            {
...
// fetch all states 
 ...
 function showallData(){
          ...
          $.ajax({
                type: 'ajax',
                method:'get',
                url: "<?php echo base_url() ?>api/Location/fetch_all_states",
                async: false,
                dataType: 'json',
                success: function(response) {
                    // body...
