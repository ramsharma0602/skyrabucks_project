...
<script type="text/javascript">
 ...
 $('#btnadd').click(function(){
   ...
   $('#myform')[0].reset();
   $('#btnSave').html('Submit');
   $('#exampleModal').find('.modal-title').text('Add State Location');
   $('#myform').attr('action','<?php echo base_url(); ?>api/State_location/add_state');
 });
...
$('#sub_category_data').on('click','.item-delete',function(){
    ...
    $('#btdelete').unbind().click(function(){
        $.ajax({
            ...
            method: 'delete',
            async: false,
            url: '<?php echo base_url(); ?>api/State_location/delete_location/'+id,
            data: {id: id},
            dataType: 'json',
            success: function(response)
            {
...
function all_category(){
 ...
 function showallData(){
            ...
            $.ajax({
                type: 'ajax',
                url: "<?php echo base_url() ?>api/State_location/fetch_all_location/",
                async: false,
                dataType: 'json',
                success: function(response) {
                    // body...
                    ...
                    for (x =0 ; x<data.length; x++) {
                      var i=x+1
                       ...
                       html+='<tr>'+
                               ...
                               '<a href="javascript:;" style="margin: 10px;" class="btn btn-sm btn-outline-danger item-delete" data="'+data[x].state_admin_id+'"><i class="fas fa-trash-alt"></i></a>'+
                               '</td>'+
                           '</tr>';
