...
<script type="text/javascript">
 ...
 $('#btnadd').click(function(){
   ...
   $('#myform').attr('action','<?php echo base_url(); ?>api/Cities_and_service_provider/add_city');
 });
...
$('#btnSave').click(function(){
   ...
   // console.log(data); 
    $.ajax({
        ...
        url: url,
        data: {
          service_provider_id:<?php echo $service_provider_id ?>,
          city_id:city_id
        },
        async: false,
        dataType: 'json',
...
$('#sub_category_data').on('click','.item-delete',function(){
    ...
    $('#btdelete').unbind().click(function(){
        $.ajax({
            ...
            method: 'delete',
            async: false,
            url: '<?php echo base_url(); ?>api/Cities_and_service_provider/delete_location/'+id,
            data: {id: id},
            dataType: 'json',
            success: function(response)
            {
...
function all_category(){
    $.ajax({
          url:"<?php echo base_url().('api/Cities_and_service_provider/fetch_all_cities');?>",
          method:"get",
          async: false,
          dataType: 'json',  
            success:function(response){
                    ...
                    for (x =0 ; x<data.length; x++) {
                      var i=x+1
                       html+='<tr>'+
                               ...
                               '<a href="javascript:;" style="margin: 10px;" class="btn btn-sm btn-outline-danger item-delete" data="'+data[x].city_service_provider_id+'"><i class="fas fa-trash-alt"></i></a>'+
                               '</td>'+
                           '</tr>';
