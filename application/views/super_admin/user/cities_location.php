  ...
  <form id="myform">
      <div class="modal-body">
            ...
            <input type="hidden" name="city">
                <div class="form-group">
                    <label for="exampleFormControlSelect2">Add Cities Location</label>
                </div>
              <select class="form-control target" id="city_id" name="city_id">
               <option value="" selected=" " hidden="">Please Select Cities Location</option>
              </select>
...
<script type="text/javascript">
 ...
 $('#btnadd').click(function(){
   ...
   $('#myform').attr('action','<?php echo base_url(); ?>api/City_location/add_city');
 });
...
$('#sub_category_data').on('click','.item-delete',function(){
    ...
    $('#btdelete').unbind().click(function(){
        $.ajax({
            ...
            method: 'delete',
            async: false,
            url: '<?php echo base_url(); ?>api/City_location/delete_location/'+id,
            data: {id: id},
            dataType: 'json',
            success: function(response)
            {
...
function all_category(){
    $.ajax({
          url:"<?php echo base_url().('api/City_location/fetch_all_cities');?>",
          method:"get",
          async: false,
          dataType: 'json',  
            success:function(response){
                 
                var html='<option value="" selected hidden>Please select Cities</option>';
                var x,i;
 ...
 function showallData(){
            ...
            $.ajax({
                type: 'ajax',
                url: "<?php echo base_url() ?>api/City_location/fetch_all_location/",
                async: false,
                dataType: 'json',
                success: function(response) {
                    // body...
                    ...
                    for (x =0 ; x<data.length; x++) {
                      var i=x+1
                       html+='<tr>'+
                               ...
                               '<td>'+
                               '<a href="javascript:;" style="margin: 10px;" class="btn btn-sm btn-outline-danger item-delete" data="'+data[x].city_sub_admin_id+'"><i class="fas fa-trash-alt"></i></a>'+
                               '</td>'+
                           '</tr>';
