...
class States_and_super_admin extends REST_Controller {
    /**
    ...
    */
    ...
    //    $this->load->database();
     ...
     public function fetch_state_get(){
       ...
       $active_super_admin= $this->session->userdata('active_super_admin');

        $option = array(
             
             'select' =>'states_and_super_admin.*,states.state_name',
             'table'=>'states_and_super_admin',
             'join'=>array(array('states'=>'states.state_id=states_and_super_admin.state_id')),
             'where' =>array('states_and_super_admin.super_admin_id' => $active_super_admin),
             'single'=>false,  
             );
...
public function add_state_post() {
          ...
          $super_admin_id =   $this->security->xss_clean($this->post("super_admin_id"));
        // checking form submission have any error or not

        if(!empty($state_id) && is_numeric($state_id)  && !empty($super_admin_id)){
          
              $con['conditions'] = array(
                  'state_id' => $state_id,);

              $userCount = $this->Crud_model->getRows('states_and_super_admin',$con,'row');
              ...
              if($userCount){
                  ...
                  if($this->Crud_model->insert('states_and_super_admin',$category,$con)){
                      // Set the response and exit
                    $this->response([
                          'status' => TRUE,
...
public function delete_states_delete($state_id='') {
 ...
 public function fetch_all_location_get() {
           ...
           $option = array(
               'select' => 'states_and_super_admin.*,states.state_name,super_admin.name',
                 'table' => 'states_and_super_admin',
                 'join'=>array(array('states'=>'states.state_id=states_and_super_admin.state_id'),array('super_admin'=>'super_admin.super_admin_id=states_and_super_admin.super_admin_id')),
