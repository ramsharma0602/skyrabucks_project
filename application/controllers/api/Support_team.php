...
class Support_team extends REST_Controller {
    /**
     ...
     public function login_post(){
       ...
       if( $email ==""){
         ...
         $this->response([
           ...
           $option = array(
               'select' => 'support_team.support_team_id,support_team.user_type_id,support_team.email,support_team.credential,user_type.user_type_id,user_type.controller_name,user_type.session_name',
                 'table' => 'support_team',
                 'join'=>array(array('user_type'=>'support_team.user_type_id=user_type.user_type_id')),
                 'where' =>array('support_team.email' => $email),
                 'single'=>true
                );
              ...
              if($result=$this->Crud_model->commonGet($option)){
                ...
                if (password_verify($password, $result->credential)) {
                   $this->response([
                      ...
                      $this->session->set_userdata($result->session_name,$result->support_team_id);
                    }
                    else{
                       $this->response([
                      'status' => FALSE,
        ...
        $con['conditions'] = array(
            ...
            $userCount = $this->Crud_model->getRows('support_team',$con,'row');
                if($userCount){
                    ...
                    if($this->Crud_model->insert('support_team',$user)){
                        // Set the response and exit
                        $this->response([
                            'status' => TRUE,
...
public function update_user_post(){
        ...
        if(!empty($user_type_id) && is_numeric($user_type_id) && !empty($user_name) && !empty($user_email) && !empty($user_phone) && $this->form_validation->valid_email($user_email) && is_numeric($user_phone) && $this->form_validation->exact_length($user_phone, 10)){
              ...
              $con['conditions'] = array(
                ...
                if($userCount){
                  ...
                  // Check if the user data is inserted
                  if($this->Crud_model->update('support_team',$user,$con)){
                      // Set the response and exit
                      $this->response([
                          'status' => TRUE,
      ...
      public function update_user_password_post() {
        ...
        if(!empty($support_team_id) && is_numeric($support_team_id) && !empty($old_password) && !empty($new_password) && !empty($confirm_password)){
        ...
        public function fetch_support_team_get($user_type_id = '') {
        ...
        // checking form submission have any error or not
           ...
           $option = array(
               'select' => 'cities_and_support_team.*,cities.city,support_team.name,support_team.email,support_team.mobile',
        ...
        if(!empty($user_type_id) ){
              ...
              $userCount = $this->Crud_model->getRows('support_team',$con,'result');
 ...
 public function change_is_active_post($support_team_id=''){
 
  $support_team_id = $this->security->xss_clean($this->get("support_team_id"));

  if (!empty($support_team_id)) {
    # code...
   ...
   $con['conditions'] = array(
    ...
    if ($userCount->is_active=='Y') {
      ...
      if($this->Crud_model->update('support_team',$product_update,$con)){
         // Set the response and exit
           $this->response([
           'status' => TRUE,
