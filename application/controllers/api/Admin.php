     ...
     public function login_post(){
       ...
       if( $email ==""){
         ...
         $this->response([
           ...
           $option = array(
               'select' => 'admin.admin_id,admin.user_type_id,admin.email,admin.credential,user_type.user_type_id,user_type.controller_name,user_type.session_name',
                 'table' => 'admin',
                 'join'=>array(array('user_type'=>'admin.user_type_id=user_type.user_type_id')),
                 'where' =>array('admin.email' => $email),
                 'single'=>true
                );
              ...
              if($result=$this->Crud_model->commonGet($option)){
                ...
                if (password_verify($password, $result->credential)) {
                   $this->response([
                      ...
                      $this->session->set_userdata($result->session_name,$result->admin_id);
                    }
                    else{
                       $this->response([
                      'status' => FALSE,
...
public function update_user_post(){
      ...
      public function update_user_password_post() {
        ...
        public function fetch_sub_admin_get($state_id = '') {
        ...
        // checking form submission have any error or not
           ...
           $option = array(
               'select' => 'cities_and_sub_admin.*,cities.city,sub_admin.name,sub_admin.email,sub_admin.mobile',
                ...
                'where'=>array('cities.state_id'=> $state_id )
                );
  ...
  if (!empty($user_id)) {
   ...
   $con['conditions'] = array(
    ...
    if ($userCount->is_active=='Y') {
      ...
      public function fetch_user_get($user_type_id = '') {
