     ...
     public function login_post(){
       ...
       if( $email ==""){
         ...
         $this->response([
           ...
           $option = array(
               'select' => 'super_admin.super_admin_id,super_admin.user_type_id,super_admin.email,super_admin.credential,user_type.user_type_id,user_type.controller_name,user_type.session_name',
                 'table' => 'super_admin',
                 'join'=>array(array('user_type'=>'super_admin.user_type_id=user_type.user_type_id')),
                 'where' =>array('super_admin.email' => $email),
                 'single'=>true
                );
              ...
              if($result=$this->Crud_model->commonGet($option)){
                ...
                if (password_verify($password, $result->credential)) {
                   $this->response([
                      ...
                      $this->session->set_userdata($result->session_name,$result->super_admin_id);
                    }
                    else{
                       $this->response([
                      'status' => FALSE,
        ...
        $con['conditions'] = array(
            ...
            $userCount = $this->Crud_model->getRows('admin',$con,'row');
                if($userCount){
                    ...
                    if($this->Crud_model->insert('admin',$user)){
                        // Set the response and exit
                        $this->response([
                            'status' => TRUE,
...
public function update_user_post(){
        ...
        if(!empty($user_type_id) && is_numeric($user_type_id) && !empty($user_name) && !empty($user_email) && !empty($user_phone) && $this->form_validation->valid_email($user_email) && is_numeric($user_phone) && $this->form_validation->exact_length($user_phone, 10)){
              ...
              $con['conditions'] = array(
                  ...
                  $userCount = $this->Crud_model->getRows('users',$con,'row');
                //   print_r($userCount);
                if($userCount){
                  // Set the response and exit
                      $user = array(
                  ...
                  // Check if the user data is inserted
                  if($this->Crud_model->update('users',$user,$con)){
                      // Set the response and exit
                      $this->response([
                          'status' => TRUE,
...
// admin fetch function 
    ...
    public function fetch_admin_get($user_type_id = '') {
        ...
        if(!empty($user_type_id) ){
              ...
              $userCount = $this->Crud_model->getRows('admin',$con,'result');
 ...
 public function change_is_active_post($admin_id=''){
 
  // $admin_id = $this->security->xss_clean($this->get("admin_id"));
  ...
  if (!empty($admin_id)) {
   ...
   $con['conditions'] = array(
    ...
    if ($userCount->is_active=='Y') {
      ...
      if($this->Crud_model->update('admin',$product_update,$con)){
         // Set the response and exit
           $this->response([
           'status' => TRUE,
