...
class States_and_sub_admin extends REST_Controller {
    /**
    ...
    */
    ...
    //    $this->load->database();
     ...
     public function fetch_state_get(){
       ...
       $active_sub_admin= $this->session->userdata('active_sub_admin');

        $option = array(
             
             'select' =>'states_and_sub_admin.*,states.state_name',
             'table'=>'states_and_sub_admin',
             'join'=>array(array('states'=>'states.state_id=states_and_sub_admin.state_id')),
             'where' =>array('states_and_sub_admin.sub_admin_id' => $active_sub_admin),
             'single'=>false,  
             );
...
public function add_state_post() {
        ...
        if(!empty($state_id) && is_numeric($state_id)  && !empty($sub_admin_id)){
          
              $con['conditions'] = array(
                  'state_id' => $state_id,);

              $userCount = $this->Crud_model->getRows('states_and_sub_admin',$con,'row');
              ...
              if($userCount){
                  ...
                  if($this->Crud_model->insert('states_and_sub_admin',$category,$con)){
                      // Set the response and exit
                    $this->response([
                          'status' => TRUE,
...
public function delete_states_delete($state_id='') {
 ...
 public function fetch_all_location_get() {
           ...
           $option = array(
               'select' => 'states_and_sub_admin.*,states.state_name,sub_admin.name',
                 'table' => 'states_and_sub_admin',
                 'join'=>array(array('states'=>'states.state_id=states_and_sub_admin.state_id'),array('sub_admin'=>'sub_admin.sub_admin_id=states_and_sub_admin.sub_admin_id')),
