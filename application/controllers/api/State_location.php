...
class State_location extends REST_Controller {
    /**
    ...
    */
    public function __construct() {
     ...
     public function add_state_post() {
        ...
        if(!empty($state_id) &&  !empty($admin_id)){
          
              $con['conditions'] = array(
                  'state_id' => $state_id,);

              $userCount = $this->Crud_model->getRows('state_admin',$con,'row');
              ...
              if($userCount){
                  ...
                  if($this->Crud_model->insert('state_admin',$category,$con)){
                      // Set the response and exit
                    $this->response([
                          'status' => TRUE,
...
public function delete_location_delete($id='') {
        ...
        if(!empty($id) ){
              ...
              if($userCount){
                  // Set the response and exit
                     if($this->Crud_model->delete('state_admin',$con)){
                      // Set the response and exit
                      $this->response([
                          'status' => TRUE,
 ...
 public function fetch_all_location_get() {
           ...
           $option = array(
               'select' => 'state_admin.*,states.state_name,admin.admin_name',
                 'table' => 'state_admin',
                 'join'=>array(array('states'=>'states.state_id=state_admin.state_id'),array('admin'=>'admin.admin_id=state_admin.admin_id')),
    ...
    public function fetch_admin_state_get() {

            // $userCount = $this->Crud_model->getRows('state_admin',$arrayname =array(),'result');

       $active_admin= $this->session->userdata('active_admin');
           $option = array(
               'select' =>'state_admin.*,states.state_name',
             'table'=>'state_admin',
             'join'=>array(array('states'=>'states.state_id=state_admin.state_id')),
             'where' =>array('state_admin.admin_id' => $active_admin),
             'single'=>false,  
...
public function delete_states_delete($state_id='') {
 ...
 public function fetch_all_states_get() {
